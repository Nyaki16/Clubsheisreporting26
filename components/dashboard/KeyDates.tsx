...
export function KeyDates({ slug }: { slug: string }) {
    ...
    <div>
      <div className="flex items-center justify-between">
        <h2 className="font-serif text-xl font-semibold text-gray-900 flex items-center gap-2">
          <CalendarDays className="w-5 h-5 text-[#4A1942]" /> Key Dates
        </h2>
        {!editing && (
          <button
            onClick={startEdit}
            className="flex items-center gap-1.5 text-sm text-[#4A1942] hover:text-[#6b2563] font-medium px-3 py-1.5 rounded-lg hover:bg-purple-50 transition-colors"
          >
            {dates.length > 0 ? <Pencil className="w-3.5 h-3.5" /> : <Plus className="w-3.5 h-3.5" />}

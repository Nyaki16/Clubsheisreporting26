...
import type { GHLData } from "@/types/dashboard";

export function CRMContent({ slug }: { slug: string }) {
  const { data: sections, loading } = useDashboardSections(slug, ["ghl"]);
  ...
  if (!ghlData) return <EmptyState message="No CRM data available for this client." />;
